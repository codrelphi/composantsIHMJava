...
public class Identite extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// Rappel: le contrôleur joue le rôle de chef d'orchestre/router/aiguilleur 
		// le contrôleur délègue la tâche d'affichage à la vue
		// this: c'est l'objet courant (l'instance de la classe que nous sommes en train d'utiliser
		// dispatcher = distribuer (en francais) à qqn/ qch
		// forward = propager
		...
		// côté servlet (controlleur), on utilise <request.setAttribute("nom", variable)
		// côté jsp (vue), on utilise <request.getAttribute("nom");
		
		//request.setAttribute(name, o);
		
		request.getRequestDispatcher("/WEB-INF/index.jsp").forward(request, response);
		
		// Construction de la page html depuis le contrôleur
		// nB: ce n'est pas une bonne manière. 
